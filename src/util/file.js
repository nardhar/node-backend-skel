 ...
 */
const loaddirSync = (dir, suffix, ignore = [], onlyFolder = []) => {
  ...
  fs.readdirSync(dir).forEach((file) => {
    ...
    if (!fs.statSync(filePath).isDirectory()) {
      ...
      if (
        ...
        // and that belongs to onlyFolder array
        && (onlyFolder.length > 0 ? onlyFolder.indexOf(file) >= 0 : true)
      ) {
